...
def __main__(fake=False, mini=False, language='nl'):
    if language == 'nl':
        s = SeqUtils2.__main__(fake=fake, constructive=True, sequence_file='test-output/sequences/words-types.p',
                               return_types=True, mini=mini, language='nl')
    elif language == 'fr':
        s = SeqUtils2.__main__(fake=fake, constructive=True, sequence_file='test-output/sequences/words-types_fr.p',
    ...
    marks = SeqUtils2.get_type_occurrences([s.type_sequences[i] for i in train_indices])
...
def store_samples(network, dataset, val_indices, device='cuda', batch_size=128, log_file='nn/val_log.tsv', marks=None):
    ...
    while batch_start < len(val_indices):
        ...
        batch_t_types = SeqUtils2.convert_many_vector_sequences_to_type_sequences(batch_y, dataset.atomic_dict)
        batch_t_types = [[t for t in batch_t_types[i] if t] for i in range(len(batch_t_types))]
        t_types.extend(batch_t_types)

        batch_p_types = SeqUtils2.convert_many_vector_sequences_to_type_sequences(prediction, dataset.atomic_dict)
        p_types.extend([[batch_p_types[i][j] for j in range(len(batch_t_types[i]))] for i in range(len(batch_t_types))])

        batch_start = batch_end

    imagined_types = SeqUtils2.get_all_unique(p_types) - SeqUtils2.get_all_unique(t_types)
...
def mark_types(dataset, train_indices, val_indices):
    ...
    type_occurrences_train = SeqUtils2.get_type_occurrences([dataset.type_sequences[i] for i in train_indices])

    type_occurrences_all = SeqUtils2.get_type_occurrences(dataset.type_sequences)
    type_occurrences_train = SeqUtils2.get_type_occurrences([dataset.type_sequences[i] for i in train_indices])
    # todo:
    ...
    type_occurrences_train = SeqUtils2.get_type_occurrences([dataset.type_sequences[i] for i in train_indices])
    type_occurrences_val = SeqUtils2.get_type_occurrences([s.type_sequences[i] for i in val_indices])
